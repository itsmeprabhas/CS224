`timescale 1ns / 1ps
module seven_seg_driver(input clk, reset, input [31:0] data_in, output reg [7:0] an, output reg [6:0] seg);
    reg [16:0] refresh_counter;
    always @(posedge clk or posedge reset) begin
        if (reset) refresh_counter <= 0; else refresh_counter <= refresh_counter + 1;
    end
    wire [2:0] digit_select = refresh_counter[16:14]; reg [3:0] current_hex;
    always @(*) begin
        case(digit_select)
